  ...
  for (var key in json.keys) {
    ...
    if (json[key] == null) {
      ...
      if (key.contains('_') || key.startsWith(r"$")) {
        ...
        fields += 'final $name? ${key.replaceAll("\$", "").camelCase};\n';
      }
      classNames.add(name);
      ...
      if ((json[key] as List).isEmpty) {
        if (key.contains('_') || key.startsWith(r"$")) {
          ...
          fields +=
              'final List<dynamic>? ${key.replaceAll("\$", "").camelCase};\n';
        }
    ...
    fieldsConstructors += 'this.${key.replaceAll("\$", "").camelCase},\n';
  }

  output += template
      .replaceAll('{className}', className)
      .replaceAll('{field}', fields.replaceAll('"\$', '"\\\$'))
      .replaceAll('{fieldsConstructor}', fieldsConstructors);
}
...
class ConvertJsonToModelScreen extends StatefulWidget {
  const ConvertJsonToModelScreen({Key? key}) : super(key: key);

  @override
  State<ConvertJsonToModelScreen> createState() =>
      _ConvertJsonToModelScreenState();
}

class _ConvertJsonToModelScreenState extends State<ConvertJsonToModelScreen> {
  String className = "";
  String input = "";
